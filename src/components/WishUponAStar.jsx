    ...
    <div className="min-h-screen bg-gradient-to-b from-[#121212] via-[#1f1b24] to-[#0f0c29] flex flex-col justify-center items-center px-4 py-12 text-center snap-start relative">
      ...
      <div className="flex flex-wrap justify-center gap-5 max-w-xs sm:max-w-md">
        {birthdayWishes.map((quote, idx) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="w-12 h-12 sm:w-14 sm:h-14 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-md border border-yellow-300/30 hover:bg-yellow-200/10 transition shadow-lg"
          >
      ...
      {selectedWish && (
        <motion.div
          ...
          className="mt-6 sm:mt-8 px-4 py-3 sm:px-6 sm:py-4 max-w-xs sm:max-w-md text-pink-100 bg-white/10 border border-pink-200/20 rounded-xl shadow-xl backdrop-blur"
        >
          <p className="text-base sm:text-lg">{selectedWish}</p>
        </motion.div>
      )}

      <ChevronDownIcon className="w-5 h-5 sm:w-6 sm:h-6 text-pink-300 mt-10 animate-bounce" />
